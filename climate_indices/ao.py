...
from .anomalies import monthly_anomalies, daily_anomalies
from .eofs import calc_eofs
from .timeavg import (multiyear_monthly_means, multiyear_daily_means)
...
def calc_ao_from_anomalies(time, lat, lon, monthly_z1000_anom,
                           start_year=None, end_year=None,
                           random_state=None):
    """Calculate AO EOF pattern and PC time series from given anomalies.
    ...
    monthly_z1000_anom : array-like, shape (n_samples, n_lat, n_lon)
        Monthly mean 1000 hPa geopotential height anomalies data.
...
def calc_ao(time, lat, lon, monthly_z1000, start_year=None, end_year=None,
            ignore_nan=True, dtype=None, min_records=None,
            random_state=None):
    """Calculate AO EOF pattern and PC time series.
    ...
    monthly_z1000 : array-like, shape (n_samples, n_lat, n_lon)
        Monthly mean 1000 hPa geopotential height data.
    ...
    https://www.cpc.ncep.noaa.gov/products/precip/CWlink/daily_ao_index/history/method.shtml
    """
    _check_input_data(time, lat, lon, monthly_z1000, 'calc_ao')
    ...
    valid_lat = lat[lat_mask]
    valid_data = monthly_z1000[time_mask, :, :]
    valid_data = valid_data[:, lat_mask, :]
    ...
    return calc_ao_from_anomalies(valid_time, valid_lat, lon, anom,
                                  start_year=start_year, end_year=end_year,
                                  random_state=random_state)


def calc_ao_index(time, lat, lon, z1000_anom, ao_eof,
                  start_year=None, end_year=None,
                  normalization=None):
    time_mask, lat_mask, _ = _get_analysis_masks(
        time, lat, lon, start_year=start_year, end_year=end_year)
    ...
    valid_time = time[time_mask]
    valid_lat = lat[lat_mask]
    valid_data = z1000_anom[time_mask, :, :]
    valid_data = valid_data[:, lat_mask, :]

    weights = _get_scos_weights(valid_lat, valid_data)
    weighted_data = weights * valid_data

    index = _project_data(weighted_data, ao_eof)
