...
try:
    ...
    dask_not_imported = False
    import climate_indices.eofs._eofs_dask_backend as eofs_dask
except ImportError:
    dask_not_imported = True
...
class TestStandardEOFsDefaultBackend(unittest.TestCase):
    ...
    def test_standard_eofs_2d_cov_no_missing_normalization_colvar(self):
        random_seed = 0
        rng = np.random.RandomState(seed=random_seed)
    ...
    def test_standard_eofs_2d_cov_no_missing_pcs_normalization_rowvar(self):
        random_seed = 0
        rng = np.random.RandomState(seed=random_seed)
    ...
    def test_standard_eofs_2d_cov_no_missing_pcs_normalization_colvar(self):
        random_seed = 0
        rng = np.random.RandomState(seed=random_seed)
...
@unittest.skipIf(dask_not_imported, 'dask not installed')
class TestDaskBackendHelpers(unittest.TestCase):
    ...
    def test_2d_to_2d_array_rowvars(self):
        ...
        X_2d = eofs_dask._to_2d_array(X, rowvar=True)

        self.assertTrue(da.all(X == X_2d))
    ...
    def test_2d_to_2d_array_colvars(self):
        ...
        X_2d = eofs_dask._to_2d_array(X, rowvar=False)
    ...
    def test_1d_to_2d_array_rowvar_fails(self):
        ...
        with self.assertRaises(ValueError):
            eofs_dask._to_2d_array(X, rowvar=True)
    ...
    def test_1d_to_2d_array_colvar_fails(self):
        ...
        with self.assertRaises(ValueError):
            eofs_dask._to_2d_array(X, rowvar=False)
    ...
    def test_3d_to_2d_array_rowvar(self):
        ...
        X_init = np.empty(observation_shape + (n_samples,))
        X_init[:, :, 0] = np.array([[1, 2, 3], [4, 5, 6]])
        X_init[:, :, 1] = np.array([[7, 8, 9], [10, 11, 12]])
        X_init[:, :, 2] = np.array([[13, 14, 15], [16, 17, 18]])

        X = da.from_array(X_init)
        ...
        self.assertTrue(da.all(X_2d == X_2d_expected))

    def test_3d_to_2d_array_colvar(self):
        n_samples = 4
        observation_shape = (2, 3)

        X_init = np.empty((n_samples,) + observation_shape)
        X_init[0] = np.array([[1, 2, 3], [4, 5, 6]])
        X_init[1] = np.array([[7, 8, 9], [10, 11, 12]])
        X_init[2] = np.array([[13, 14, 15], [16, 17, 18]])
        X_init[3] = np.array([[19, 20, 21], [22, 23, 24]])
    ...
    def test_1d_to_nd_array_fails_rowvar(self):
        ...
        X = da.from_array(_random_anomalies(n_samples, random_state=rng,
                                            rowvar=True))

        with self.assertRaises(ValueError):
            eofs_dask._to_nd_array(X, target_shape, rowvar=True)
    ...
    def test_1d_to_nd_array_fails_colvar(self):
        ...
        with self.assertRaises(ValueError):
            eofs_dask._to_nd_array(X, target_shape, rowvar=False)
    ...
    def test_2d_to_2d_array_rowvar(self):
        ...
        X_nd = eofs_dask._to_nd_array(X, target_shape, rowvar=True)

        self.assertTrue(da.all(X_nd == X))
    ...
    def test_2d_to_2d_array_colvar(self):
        ...
        X_nd = eofs_dask._to_nd_array(X, target_shape, rowvar=False)
    ...
    def test_2d_to_4d_rowvar(self):
        ...
        self.assertTrue(da.all(X_nd_expected == X_nd))
    ...
    def test_center_2d_data_no_missing_rowvar(self):
        ...
        X = da.from_array(rng.uniform(size=(n_features, n_samples)))

        initial_mean = X.mean(axis=1)
        self.assertTrue(initial_mean.shape == (2,))
        self.assertTrue(da.all(initial_mean != 0))

        X_anom = eofs_dask._center_data(X, axis=1,
                                        skipna=False)
        final_mean = X_anom.mean(axis=1)
        self.assertTrue(np.allclose(final_mean, 0))
    ...
    def test_center_2d_data_no_missing_colvar(self):
        ...
        X = da.from_array(rng.uniform(size=(n_samples, n_features)))

        initial_mean = X.mean(axis=0)
        self.assertTrue(initial_mean.shape == (4,))
        self.assertTrue(da.all(initial_mean != 0))

        X_anom = eofs_dask._center_data(X, axis=0,
                                        skipna=False)

        final_mean = X_anom.mean(axis=0)
        self.assertTrue(da.allclose(final_mean, 0))

    def test_center_2d_data_missing_rowvar(self):
        X = da.from_array(np.array([[1, 2, np.NaN, 6],
                                    [3, 4, 5, 6],
                                    [np.NaN, np.NaN, np.NaN, 1]]))

        X_anom = eofs_dask._center_data(X, axis=1,
                                        skipna=True)
        X_anom = X_anom.compute()
        ...
        self.assertTrue(da.all(da.isnan(X_anom) == da.isnan(X_anom_expected)))

        nonmiss_mask = ~da.isnan(X_anom_expected)
        self.assertTrue(da.all(X_anom_expected[nonmiss_mask] ==
                               X_anom[nonmiss_mask]))

    def test_center_2d_data_missing_colvar(self):
        X = da.from_array(np.array([[3, np.NaN], [np.NaN, 4]]))

        X_anom = eofs_dask._center_data(X, axis=0, skipna=True)
        X_anom = X_anom.compute()
    ...
    def test_fixed_missing_values_check_rowvar(self):
        ...
        self.assertTrue(eofs_dask._has_fixed_missing_values(X, rowvar=True))
        ...
        self.assertFalse(eofs_dask._has_fixed_missing_values(
            X, rowvar=True))
    ...
    def test_fixed_missing_values_check_colvar(self):
        ...
        self.assertTrue(eofs_dask._has_fixed_missing_values(
            X, rowvar=False))
