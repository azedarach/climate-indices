...
def plot_nao_index(index, cpc_times=None, cpc_index=None,
    ...
    ax.plot(valid_times, valid_index, 'b-', label='NAO index')
    ...
    if cpc_times is not None and cpc_index is not None:
        ...
        ax.plot(
            ...
            label='CPC NAO index (r = {:.2f})'.format(corr_coeff))
...
def main():
    ...
    with xr.open_dataset(args.datafile) as ds:
        ...
        if not args.no_show_plots or args.index_plot_output_file:
            plot_nao_index(index, cpc_times=cpc_times, cpc_index=cpc_index,
