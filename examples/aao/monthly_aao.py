...
def parse_cmd_line_args():
    parser = argparse.ArgumentParser(
        description='Calculate AAO pattern and monthly index from daily data')
...
def main():
    ...
    monthly_clim = multiyear_monthly_means(
        ...
        monthly_z700[monthly_base_period_mask])

    monthly_z700_anom = monthly_anomalies(
        monthly_times, monthly_z700, monthly_clim)

    normalization = aao_pcs.std(ddof=1)

    monthly_aao_index_times, monthly_aao_index = calc_aao_index(
        monthly_times, lat, lon, monthly_z700_anom,
        aao_eofs, normalization=normalization)

    if args.index_output_file:
        write_aao_index(
            monthly_aao_index_times, monthly_aao_index, args.index_output_file)
