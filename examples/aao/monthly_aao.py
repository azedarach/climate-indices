...
from climate_indices.aao import (calculate_monthly_region_anomalies,
                                 calculate_annual_eof, calculate_aao_pc_index)
...
def plot_aao_index(index, cpc_times=None, cpc_index=None,
    ...
    ax.plot(valid_times, valid_index, 'b-', label='AAO index')
    ...
    if cpc_times is not None and cpc_index is not None:
        ...
        ax.plot(
            ...
            label='CPC AAO index (r = {:.2f})'.format(corr_coeff))
...
def parse_cmd_line_args():
    parser = argparse.ArgumentParser(
        description='Plot daily AAO index')

    parser.add_argument(
        'datafile', help='datafile containing 700 hPa geopotential heights')
    parser.add_argument(
...
def main():
    ...
    with xr.open_dataset(args.datafile) as ds:
        ...
        monthly_anom_data, _ = calculate_monthly_region_anomalies(
            hgt_data, climatology=clim)

        index = calculate_aao_pc_index(
            monthly_anom_data, annual_eofs['eofs'],
        ...
        if not args.no_show_plots or args.index_plot_output_file:
            plot_aao_index(index, cpc_times=cpc_times, cpc_index=cpc_index,
